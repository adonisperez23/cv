<template>
  <div>
    <h5 class="text-deep-purple-6">Conocimientos y Habilidades</h5>
    <ul>
      <li>Proactivo</li>
    </ul>
    <ul>
      <li>Trabajo en equipo</li>
    </ul>
    <ul>
      <li>Responsable</li>
    </ul>
    <ul>
      <li>Capacidad de aprendizaje</li>
    </ul>
    <ul>
      <li>Manejo de Microsoft Word intermedio/avanzado</li>
    </ul>
    <ul>
      <li>Manejo de Microsoft Excel intermedio/avanzado.</li>
    </ul>
    <ul>
      <li>Manejo de Microsoft PowerPoint intermedio</li>
    </ul>
    <ul>
      <li>Ingles instrumental.</li>
    </ul>
    <ul>
      <li>HTML, CSS y JS básicos/intermedio</li>
    </ul>
    <ul>
      <li>Python básico/intermedio.</li>
    </ul>
    <ul>
      <li>Django y Django restframework intermedio</li>
    </ul>
    <ul>
      <li>Manejo de Adobe Illustrator CC básico/intermedio.</li>
    </ul>
    <ul>
      <li>Manejo de sistemas operativos Windows XP, Vista,
        7, 8, 10 y Software Libre.</li>
    </ul>

  </div>
</template>

<script>
export default {
  // name: 'ComponentName',
  setup () {
    return {}
  }
}
</script>
